...
#define NTHR   10    			/* number of threads */
#define NUMNUM 64000000L		/* number of numbers to sort */
#define TNUM   (NUMNUM/NTHR)	/* number of elements to sort per thread */
...
int main()
{
    ...
    for (i = 0; i < NTHR; i++) {
        ...
        err = pthread_create(&tid[i], NULL, threadFunction, (void *)startOffset);
        if (err != 0){
